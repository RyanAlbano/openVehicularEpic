...
public class VE extends Application {
 ...
 public static final List<String> maps = new ArrayList<>(Arrays.asList(SL.basic, "lapsGlory", SL.checkpoint, "gunpowder", "underOver", SL.antigravity, "versus1", "versus2", "versus3", "trackless", "desert", "3DRace", "trip", "raceNowhere", "moonlight", "bottleneck", "railing", "twisted", "deathPit", "falls", "pyramid", "combustion", "darkDivide", "arctic", "scenicRoute", "winterMode", "mountainHop", "damage", "cavern", "southPole", "aerialControl", "matrix", "mist", "vansLand", "dustDevil", "forest", "columns", "zipCross", "highlands", "coldFury", SL.tornado, "volcanic", SL.tsunami, SL.boulder, "sands", SL.meteor, "speedway", "endurance", "tunnel", "circle", "circleXL", "circles", "everything", "linear", "maze", "xy", "stairwell", "immense", "showdown", "ocean", "lastStand", "parkingLot", "city", "machine", "military", "underwater", "hell", "moon", "mars", "sun", "space1", "space2", "space3", "summit", "portal", "blackHole", "doomsday", "+UserMap & TUTORIAL+"));
 ...
 public enum UI {//UserInterface
  ...
  private enum colorOpacity {
   ;
   private static final double minimal = .5, maximal = .75;
  }

  static String getUnitDistanceName() {
 ...
 public enum Map {
    ...
    for (; (s = BR.readLine()) != null; ) {
     ...
     if (status == VE.Status.mapLoadPass2) {
      ...
      if (s.startsWith("trees(")) {
       ...
       for (n = 0; n < U.getValue(s, 0); n++) {
        TE.mounds.add(new FrustumMound(U.randomPlusMinus(800000), U.randomPlusMinus(800000), 0, 100 + U.random(400.), U.random(100.), U.random(200.), true, false, false));
       }
      }
      Fire.load(s);
      Boulder.load(s);
     ...
     if (status == VE.Status.mapLoadPass3) {
      ...
      if (U.startsWith(s, "(", SL.strip + "(", SL.curve + "(")) {
       ...
       if (U.equals(name, "Columns Condemn")) {
        ...
        random[0] *= 1000;
        random[2] *= 1000;
       } else if (name.equals(SL.Maps.phantomCavern)) {
  ...
  static void addTransparentNodes() {
   ...
   for (int n = E.Fog.spheres.size(); --n >= 0; ) {
    U.Nodes.add(E.Fog.spheres.get(n));
   }
  }
 ...
 private static void loadVE(Stage stage) {
  Thread loadVE = new Thread(() -> {
   try {
    ...
    U.Images.load(images, SL.metal);
    U.Images.load(images, SL.brightmetal);
    U.Images.load(images, SL.grid);
 ...
 enum Viewer {
  ;
  private static boolean inUse;
  private static double Y, Z, XZ, YZ, heightChange, depthChange;
  ...
  enum Vehicle {
   ...
   private static void run(boolean gamePlay) {
    ...
    if (vehicles.get(0) != null) {
     ...
     vehicles.get(0).runRender(gamePlay);
     U.font(.02);
  ...
  private static void runMapViewer(boolean gamePlay) {
   ...
   Y += heightChange * UI.movementSpeedMultiple * tick;
   Camera.Y = Y;
   Camera.Z += depthChange * U.cos(Camera.XZ) * UI.movementSpeedMultiple * tick;
   Camera.X += depthChange * U.sin(Camera.XZ) * UI.movementSpeedMultiple * tick;
   U.rotate(Camera.camera, Camera.YZ, -Camera.XZ);
   Camera.rotateXY.setAngle(-Camera.XY);
   Camera.setAngleTable();
 ...
 public enum Match {
  ...
  private static void runHUDBlocks(Vehicle V) {
   ...
   U.fillRGB(1);
   U.font(.0075);
   U.text("(" + UI.getUnitDistanceName() + ")", .025, .875);
  ...
  public static void processStunt(Vehicle V) {
   if (V.index == vehiclePerspective && V.P.stuntTimer > V.stuntLandWaitTime() && V.P.stuntReward > 0) {
