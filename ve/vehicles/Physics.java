...
public class Physics {
 ...
 boolean onFlatTrackPlane, onMoundSlope, onVolcano;
 ...
 void resetLocalGround() {
  localGround = E.Ground.level + (atPoolXZ ? Pool.depth : 0);
 }
 ...
 private void crash(double power, boolean vehicleCollide) {
  ...
  if (fragilityBased > (vehicleCollide ? 0 : 50)) {
   V.addDamage(fragilityBased * 3 * VE.tick);
       ...
       for (Vehicle vehicle : VE.vehicles) {
        ...
        !vehicle.destroyed && !vehicle.reviveImmortality && !vehicle.phantomEngaged) {
         ...
         ((vehicle.Z <= V.Z && U.cos(V.XZ) <= 0) || (vehicle.Z >= V.Z && U.cos(V.XZ) >= 0)))/*<-inZ*/ {
          double amount = 10 * V.energyMultiple * VE.tick;
          if (disintegrate) {
           for (VehiclePart part : vehicle.parts) {
            part.deform();
           }
           hitCheck(vehicle);
           VE.Match.scoreDamage[greenTeam ? 0 : 1] += replay ? 0 : amount;
          }
 ...
 static boolean advancedCollisionCheck(Core moving, Core stationary, double collideAt) {//<-Stationary core may not always be stationary in actuality, but good enough
  if (U.distance(moving, stationary) < collideAt) {
   return true;
  }
  double
  behindX = moving.X - (moving.speedX * VE.tick),
  behindY = moving.Y - (moving.speedY * VE.tick),
  behindZ = moving.Z - (moving.speedZ * VE.tick),
 ...
 public void run() {
  ...
  if (V.explosionsWhenDestroyed > 0 && !V.isIntegral() && !V.destroyed) {
   ...
   if (V.VA.deathExplode != null) {//<-Nukes don't have this
    V.VA.deathExplode.play(Double.NaN, V.VA.distanceVehicleToCamera);
   }
  ...
  if (!V.isFixed()) {
   ...
   if (mode.name().startsWith(SL.drive) || V.phantomEngaged) {
    if (!flipped()) {
     ...
     V.XZ += speedXZ * turnSpeed * angleToSteerVelocity * VE.tick;
    }
   ...
   if (!V.phantomEngaged) {
    ...
    runSetTerrainFromTrackPlanes(gravityCompensation);//<-terrainProperties set here
    resetLocalGround();
 ...
 private void runGroundConnect(boolean crashLand, double bounceBackForce, double flatPlaneBounce) {
  if (!onFlatTrackPlane) {
   ...
   significantDown = speedY > -E.gravity * VE.tick;//<-Should prevent vehicles from 'hugging' mounds instead of taking off fully
   boolean flipped = flipped();
   for (Wheel wheel : V.wheels) {
    if (wheel.inLocalGround) {
     connected = true;
     if (significantDown) {
 ...
 private void runDriveVelocity() {
  if (mode.name().startsWith(SL.drive) || V.phantomEngaged) {
   boolean flipped = flipped();
   double drift = V.handbrake && !flipped && !U.equals(VE.Map.name, SL.Maps.theMaze, SL.Maps.XYLand) ? V.grip * .25 * Math.abs(driftXZ - V.XZ) : 0,
   ...
   setGrip = Math.max(setGrip * V.energyMultiple * VE.tick, 0);
 ...
 private void runAerialControl() {
  ...
  if (stuntSpeedYZ > 0) {
   V.Y -= V.airPush * stuntSpeedYZ * VE.tick;
  }
  boolean steerByMouse = V.steerByMouse && (V.handbrake ? stuntSpeedXZ : stuntSpeedXY) * -40 < VE.Mouse.steerX;
  ...
  if ((V.turnR && !V.turnL) || steerByMouse) {
   if (V.handbrake) {
    if (V.airAcceleration < Double.POSITIVE_INFINITY) {
     stuntSpeedXZ += V.airAcceleration * VE.tick;
    }
    ...
    if (V.airAcceleration < Double.POSITIVE_INFINITY) {
     stuntSpeedXY += V.airAcceleration * VE.tick;
    }
 ...
 private void runFlight(double turnAmount, double turnSpeed) {
  if (mode == Mode.fly) {
   if (V.handbrake) {
    V.XZ += speedXZ * turnSpeed * angleToSteerVelocity * polarity * VE.tick;
    if (Math.abs(V.XY) < turnAmount * angleToSteerVelocity * VE.tick) {
     V.XY = 0;
    } else {
     V.XY += (V.XY < 0 ? 1 : V.XY > 0 ? -1 : 0) * turnAmount * angleToSteerVelocity * VE.tick;
    }
   } else {
    double amountXY = speedXZ * airAngleToSteerVelocity * VE.tick;
    V.XY -= amountXY;
    stuntXY -= amountXY;
   }
   //DONE ADJUSTING V.XY
   ...
   amountYZ = speedYZ * halfAngleToSteer * U.cos(V.XY) * VE.tick;
   V.YZ += amountYZ;
   stuntYZ -= amountYZ;
   //DONE ADJUSTING V.YZ
   double cosYZ = U.cos(V.YZ);
   if (!V.handbrake && V.engine != Vehicle.Engine.powerjet) {
    double amountXZ = speedYZ * halfAngleToSteer * sinXY * polarity * VE.tick;
    V.XZ -= amountXZ;
    stuntXZ -= amountXZ;
   }
   double amountXZ = (speed < 0 ? -5 : 5) * sinXY * polarity * VE.tick;
   V.XZ -= amountXZ;
   stuntXZ -= amountXZ;
  ...
  if (inPool && !onFlatTrackPlane) {//<-onFlatTrackPlane check is risky, but should work for now
   ...
   if (!V.reviveImmortality) {
    if (Pool.type == Pool.Type.lava) {
     ...
     V.addDamage(Pool.type == Pool.Type.acid ? .0025 * V.durability * VE.tick : 0);
    }
   }
 ...
 private void runTrackPlaneInteraction(boolean crashLand, double bounceBackForce, double flatPlaneBounce, double gravityCompensation) {
   ...
   if (!trackPart.trackPlanes.isEmpty() && U.distanceXZ(V, trackPart) < trackPart.renderRadius + V.renderRadius) {
    for (TrackPlane trackPlane : trackPart.trackPlanes) {
     ...
     if (!gate && !antiGravity) {
      ...
      boolean criterion = V.Y >= trackY || speedY >= 0 || Math.abs(speed) < E.gravity * 4 * VE.tick,
  ...
  if (angledSurface()) {
   V.Y = Math.min(V.Y, getNetClearancedY());
  }
 }
     ...
     if (distance < radiusBottom && Math.abs(V.Y + clearance - ((FM.Y - (moundHeight * .5)) + gravityCompensation)) <= moundHeight * .5) {
      ...
      if (V.Y >= finalHeight) {
       ...
       if (flipped) {
        ...
        V.XY += (baseAngle * U.sin(vehicleMoundXZ) - V.XY) * valueAdjustSmoothing * VE.tick;
        V.YZ += (-baseAngle * U.cos(vehicleMoundXZ) - V.YZ) * valueAdjustSmoothing * VE.tick;
       }
       V.Y = finalHeight;
 ...
 private void runSpeedConstraints() {
  if (mode.name().startsWith(SL.drive) || V.phantomEngaged) {
   if (V.grip <= highGrip && !flipped() && Math.abs(speed) > netSpeed && Math.abs(Math.abs(speed) - netSpeed) > Math.abs(speed) * .5 && !terrainProperties.contains(SL.Thick(SL.ice))) {
    speed *= .5;
   }
 ...
 double getNetClearancedY() {
  return (V.wheels.get(0).Y + V.wheels.get(1).Y + V.wheels.get(2).Y + V.wheels.get(3).Y) * .25 - (clearance * U.cos(V.YZ) * U.cos(V.XY));//<-U.cos's correct upside-down landings, and is basically a counter to the wheels' positions being skewed by existing 'clearance'
 }
 ...
 void runSetCorePosition(boolean onBouncy) {
  ...
  V.X += speedX * VE.tick;
  V.Z += speedZ * VE.tick;
  V.Y = Math.min(V.Y, localGround);//<-Prevents vehicles from going underground during hard ground hits
 }

 private void runSetWheelXYZ() {
  double primeX = speedX * VE.tick, primeY = speedY * VE.tick, primeZ = speedZ * VE.tick;
  for (Wheel wheel : V.wheels) {
   wheel.inLocalGround = false;
