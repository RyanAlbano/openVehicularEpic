...
public enum TE {//TrackElements
 ...
 public static void addTrackPart(String s, int listNumber, double summedPositionX, double summedPositionY, double summedPositionZ, double optionalRotation) {
  ...
  instanceScale[1] = VE.Map.name.equals(SL.Maps.ghostCity) && listNumber == getTrackPartIndex(Models.cube.name()) && instanceSize == 10000 ? 1 + U.random(3.) : instanceScale[1];
  ...
  if (listNumber == getTrackPartIndex(Models.checkpoint.name())) {
   if (VE.Map.name.equals(SL.Maps.highlands)) {
    if (U.random() < .5) {
     if (U.random() < .5) {
   ...
   } else if (VE.Map.name.equals(SL.Maps.ghostCity)) {
    if (U.random() < .5) {
     rotation = 0;
 ...
 public static void setVehicleMatchStartPlacement(Vehicle V) {
  ...
  if (VE.Map.name.equals("Vicious Versus V3") && VE.vehiclesInMatch > 1) {
   ...
   V.Y = -4000;
  } else if (VE.Map.name.equals(SL.Maps.tunnelOfDoom)) {
   if (!V.explosionType.name().contains(Vehicle.ExplosionType.nuclear.name()) && !V.isFixed()) {
    V.X = U.randomPlusMinus(700.);
    V.Z = U.random(6000.) - U.random(10000.);
   }
  } else if (VE.Map.name.equals(SL.Maps.everybodyEverything)) {
   V.X = U.random() < .5 ? -2000 : 2000;
   V.Z = U.randomPlusMinus(20000.);
  } else if (VE.Map.name.equals(SL.Maps.theMaze)) {
   if (!V.isFixed()) {
    V.X = V.Z = 0;
   }
  } else if (VE.Map.name.equals(SL.Maps.volcanicProphecy)) {
   V.X *= 2;
   V.Z *= 2;
  } else if (VE.Map.name.equals(SL.Maps.speedway2000000)) {
  ...
  } else if (VE.Map.name.equals(SL.Maps.summitOfEpic)) {
 ...
 public static void runVehicleInteraction(Vehicle V, boolean replay) {
  if (!points.isEmpty()) {
   ...
   (U.distanceXZ(V, P) < 500 || (V.AI.skipStunts && P.type != Point.Type.mustPassIfClosest) || (!checkpoints.isEmpty() && !VE.Map.name.equals(SL.Maps.devilsStairwell) && U.distance(V, checkpoints.get(V.checkpointsPassed)) <= U.distance(P, checkpoints.get(V.checkpointsPassed))))) {
    V.point++;
   }
  }
  if (!checkpoints.isEmpty() && !V.phantomEngaged) {
   double checkSize = VE.Map.name.equals(SL.Maps.circleRaceXL) ? V.P.speed : 0;
