...
enum GroundPlate {
 ...
 static void run() {
  if (!instances.isEmpty()) {
   double radius = E.Pool.C[0].getRadius() - instances.get(0).getRadius();
 ...
 static class Instance extends Cylinder {
  ...
  void run(double radius) {
   ...
   if (y > Camera.Y && (!E.Pool.exists || U.distance(X, E.Pool.X, Z, E.Pool.Z) > radius) && U.render(X, y, Z, -getRadius())) {
